...
## Проблемы 🚩
Вполне возможно, что у вас сломаются очереди RabbitMQ, например из-за неправильно последовательности заупска приложения.
Приметы:
- NoneType из БД при получении чего-либо оттуда
- При выведении задачек по сложности (hard, medium, easy) они путаются между собой (hard, meduim и easy)

**Решение:**
Просто заново запустите приложение, с удалением контейнеров:
```bash
docker compose down -v
docker compose up
```
```bash
source venv/bin/activate
python -m consumer.__main__
```
```bash
sudo su
```
Введите пароль и внутри данной консоли запустите:
```bash
source venv/bin/activate
python -m src.app
```
